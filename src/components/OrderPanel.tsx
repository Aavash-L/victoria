...
const OrderPanel: React.FC<OrderPanelProps> = ({ items, isOpen, onClose, onRemove }) => {
  ...
  const calculateTotal = () => {
    // Logic: 2 items for S/150, others at normal price
    const count = items.length;
    const pairCount = Math.floor(count / 2);
    const singletonCount = count % 2;
    
    // For singletons, we use their actual price. 
    // To simplify (and favor the user), we take the lowest price if there are differences.
    // In our catalog, it's 85 or 90.
    let total = pairCount * 150;
    
    if (singletonCount > 0) {
      // Find the last item that wasn't paired
  ...
  const handleOrder = async (platform: 'whatsapp' | 'instagram') => {
    ...
    try {
      ...
      const links = {
        whatsapp: 'https://wa.me/5198410034663',
  ...
  return (
    <div className={`fixed inset-0 z-[100] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      {/* Content */}
      <div className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-[2.5rem] flex flex-col max-h-[92vh] transition-transform duration-500 transform ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="w-12 h-1.5 bg-brand-pink-soft/30 rounded-full mx-auto my-4 flex-shrink-0" onClick={onClose} />
        
        <div className="px-6 pb-4 flex-shrink-0">
          <h3 className="text-xl font-bold flex items-center justify-between">
            Tu Pedido ✨
            <button onClick={onClose} className="p-2 -mr-2 text-foreground-custom/40">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between py-4 border-b border-brand-pink-light last:border-0">
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <button onClick={() => onRemove(item.id)} className="p-2 -mr-2 text-rose-300 hover:text-rose-500 transition-colors">
        ...
        <div className="px-6 py-6 bg-white border-t border-brand-pink-light shadow-[0_-10px_20px_rgba(0,0,0,0.02)] rounded-t-3xl flex-shrink-0">
          <div className="bg-brand-pink-light/30 p-4 rounded-2xl mb-6">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-medium text-foreground-custom/60 uppercase tracking-wider">Total estimado</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button 
              ...
              className="flex items-center justify-center gap-3 bg-[#25D366] text-white py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 shadow-md relative overflow-hidden"
            >
