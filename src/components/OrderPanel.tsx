...
const OrderPanel: React.FC<OrderPanelProps> = ({ items, isOpen, onClose, onRemove }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      {/* Content */}
      <div className={`absolute bottom-0 left-0 right-0 bg-surface rounded-t-[2.5rem] flex flex-col max-h-[92vh] transition-transform duration-500 transform border-t border-glass-border ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="w-12 h-1.5 bg-foreground-custom/10 rounded-full mx-auto my-4 flex-shrink-0 cursor-pointer" onClick={onClose} />
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between py-4 border-b border-glass-border last:border-0">
              <div className="flex flex-col pr-4">
                <span className="font-bold text-sm leading-tight text-foreground-custom">{item.name}</span>
                <span className="text-[9px] uppercase tracking-widest text-foreground-custom/30 mt-1">{item.type}</span>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <span className="font-bold text-accent-rose text-sm">S/{item.price}</span>
                <button onClick={() => onRemove(item.id)} className="p-2 -mr-2 text-foreground-custom/20 hover:text-sale-red transition-colors">
        ...
        <div className="px-6 py-6 bg-surface-light border-t border-glass-border rounded-t-3xl flex-shrink-0">
          <div className="glass-card p-4 rounded-2xl mb-6">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs font-medium text-foreground-custom/40 uppercase tracking-wider">Total estimado</span>
          ...
          <p className="text-[9px] text-center mt-4 text-foreground-custom/20 uppercase tracking-[0.2em] font-black">
